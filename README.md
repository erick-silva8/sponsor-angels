# 📍 Fatec Diadema – 1º Semestre (2025)

📘 Projeto desenvolvido nas disciplinas:

• Engenharia de Software I – Prof. Lucio Nunes <br>
• Desenvolvimento Web I – Prof. Dr. Bruno Zolotareff <br>
• Design Digital – Profª Patrícia Gallo <br>

![Preview do Projeto](/images/screenshot-readme.png)

> O Sponsor Angels é uma plataforma que conecta empresas a instituições de ensino da área de tecnologia, permitindo que alunos tenham acesso a itens tecnológicos essenciais para sua formação — de forma segura, controlada e transparente.

---

## 🚀 **Visite o Projeto**
🔗 [Acesse o Projeto Online]([https://sponsor-angels.netlify.app/])

---

## 🧠 **Sobre o Projeto**
O **Nosso propósito** reduzir desigualdade tecnológica e fortalecer a **formação profissional** de futuros talentos do setor.

🎯 **Objetivos**

- Facilitar doações tecnológicas entre empresas e instituições.
- Criar um ecossistema confiável para rastrear, validar e distribuir itens.
- Garantir que alunos de cursos tecnológicos tenham acesso a ferramentas adequadas.
- Proporcionar às empresas vantagens legais e reconhecimento institucional.

🧭 **Base Legal**

O Sponsor Angels se apoia em:

- Lei do Bem (Lei 11.196/2005) – Incentivo fiscal para empresas que investem em inovação e apoio tecnológico.
- Política Nacional de Ciência, Tecnologia e Inovação.

- ODS 4 – Educação de Qualidade
- ODS 9 – Indústria, Inovação e Infraestrutura
- ODS 10 – Redução de Desigualdades

---

## 🧩 **Tecnologias Utilizadas**

<div align="center">

| **Front-End** | **Design e Organização** | **Outros Recursos** |
|:--------------:|:-----------------------:|:-------------------:|
| <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45"/> | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/photoshop/photoshop-plain.svg" width="45"/> | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45"/> |
| <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45"/> | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/illustrator/illustrator-plain.svg" width="45"/> | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" width="45"/> |
| <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45"/> | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45"/> | 🌐 Azure Static Web Apps |

</div>

---

## 📱 **Principais Recursos**
✅ Layout totalmente **responsivo**  
✅ Estrutura semântica em **HTML5**  
✅ Estilização moderna com **CSS3** (gradientes, sombras e transições)
✅ Biblioteca Bootstrap
✅ Interatividade via **JavaScript puro**  
✅ Scrollbar e efeitos visuais personalizados  

---

## ⚙️ **Como Rodar Localmente**
```bash
# Clone o repositório
git clone https://github.com/elarssen-design/miv-elarssen-code-solutions.git

# Entre na pasta do projeto
cd miv-elarssen-code-solutions

# Abra no VS Code
code .
